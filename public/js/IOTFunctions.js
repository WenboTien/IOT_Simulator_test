...
function periodicMovement() { //every 250 milliseconds (.25 seconds) move the Robot
    ...
    WSsendSensorInfo();
    if(isConnectedWS) {
        var t = setTimeout(periodicMovement, 40-speedUp); //Frequency Based on test computers. User slider for speed
    }
...
function getSensorInfo() {
    ...
    speedUp = Number(document.getElementById("speedSetting").value);
	distanceToMove = 5+speedUp/2 //Speed updated upon sensor update
...
function startServer(){
    ...
    connection.onmessage = function(evt){
		//if(firstMessage == true){firstMessage = false; return;}
        var received = evt.data;
        var jsobject = JSON.parse(received);
		if(jsobject.servos.length == 1) { //Use of "Move at Power" or "Turn by Degrees" blocks
			if(Boolean(jsobject.servos[0].isTurn)) { //Turn by degrees
				if(jsobject.servos[0].servoSpeed > 0) //right
				{
					headingRightTurn();
					for(var i = 1; i < Math.abs(jsobject.servos[0].servoSpeed)/90; i++) {
						setTimeout(headingRightTurn,200*i);
					}
					leftWheelPower = 0;
					rightWheelPower = 0;
					
				}
				else if(jsobject.servos[0].servoSpeed < 0) //left
				{
					headingLeftTurn();
					for(var i = 1; i < Math.abs(jsobject.servos[0].servoSpeed)/90; i++) {
						setTimeout(headingLeftTurn, 200*i);
					}
					leftWheelPower = 0;
					rightWheelPower = 0;
				}
		...
		else if (jsobject.servos.length == 2) {
			leftWheelPower = Number(jsobject.servos[0].servoSpeed);
			...
			alert("Unhandled number of servo inputs: " + jsobject.servos.length);
		}
